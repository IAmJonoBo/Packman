...
function hasSuiteOwnedFiles(relativePaths: string[]): boolean {
  return (
    relativePaths.includes(".github/copilot-instructions.md") ||
    relativePaths.includes(".vscode/settings.json")
  );
}

export async function syncPackReadmes(
  sourcePath: string,
  apply = false,
  ...
  try {
    for (const packRoot of packRoots) {
      ...
      const promptFiles = (
        await fg(".github/prompts/**/*.prompt.md", { cwd: packRoot, dot: true })
      ).sort((a, b) => a.localeCompare(b));
      const agentFiles = (
        await fg(".github/agents/**/*.agent.md", { cwd: packRoot, dot: true })
      ).sort((a, b) => a.localeCompare(b));
      ...
      const manifest = await readJson<ManifestLike>(
        path.join(packRoot, "PACK_MANIFEST.json"),
      );
      const installMode =
