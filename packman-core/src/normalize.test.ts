...
describe('normalizePack', () => {
  it('generates README and manifest when missing', async () => {
    ...
    expect(preview.changes.some((change) => change.toPath.endsWith('README.md'))).toBe(true);
    expect(preview.changes.some((change) => change.toPath.endsWith('PACK_MANIFEST.json'))).toBe(true);
    expect(preview.changes.some((change) => change.toPath.endsWith('my-prompt.prompt.md'))).toBe(true);
    expect(preview.changes.some((change) => change.toPath.endsWith('my-prompt-prompt.md'))).toBe(false);
