...
if not os.path.isdir(DIR):
  print("ERROR: missing", DIR); raise SystemExit(1)
for dp,_,fns in os.walk(DIR):
  for fn in fns:
    if not fn.endswith(".instructions.md"): continue
    p=os.path.join(dp,fn)
    s=open(p,"r",encoding="utf-8",errors="ignore").read()
    m=FM.match(s)
    if not m: errs.append(f"{os.path.relpath(p,ROOT)}: missing frontmatter"); continue
    fm=m.group(1)
    for k in REQ:
