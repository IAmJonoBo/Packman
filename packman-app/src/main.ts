...
const sourcePathLabel = document.querySelector<HTMLParagraphElement>('#source-path');
const targetPathLabel = document.querySelector<HTMLParagraphElement>('#target-path');
const output = document.querySelector<HTMLPreElement>('#output');
const strictToggle = document.querySelector<HTMLInputElement>('#strict-toggle');
const collisionMode = document.querySelector<HTMLSelectElement>('#collision-mode');
const collisionDecisions = document.querySelector<HTMLTextAreaElement>('#collision-decisions');
const collisionSummary = document.querySelector<HTMLDivElement>('#collision-summary');
...
function extractCollisions(payload: unknown): CollisionRow[] {
  ...
  const report = payload as { output?: { plans?: Array<{ collisions?: Array<unknown> }> } };
  ...
  for (const plan of plans) {
    ...
    for (const collision of plan.collisions) {
      ...
      const entry = collision as { relativePath?: unknown; availableActions?: unknown };
      ...
      rows.push({
        ...
        availableActions: Array.isArray(entry.availableActions)
          ? entry.availableActions.filter((item): item is string => typeof item === 'string')
          : [],
      });
    }
  }
...
function renderCollisionSummary(payload: unknown): void {
  ...
  const header = '<table><thead><tr><th>Path</th><th>Actions</th></tr></thead><tbody>';
  const body = rows
    .map((row) => `<tr><td>${row.relativePath}</td><td>${row.availableActions.join(', ')}</td></tr>`)
    .join('');
...
async function pickDirectory(kind: 'source' | 'target'): Promise<void> {
  const selection = await open({
    directory: true,
    multiple: false,
    title: kind === 'source' ? 'Select source pack folder' : 'Select install target folder',
  });
...
document.querySelector<HTMLButtonElement>('#pick-source')?.addEventListener('click', () => {
  void pickDirectory('source');
});

document.querySelector<HTMLButtonElement>('#pick-target')?.addEventListener('click', () => {
  void pickDirectory('target');
});

document.querySelector<HTMLButtonElement>('#save-decisions')?.addEventListener('click', () => {
...
document.querySelector<HTMLButtonElement>('#load-decisions')?.addEventListener('click', () => {
  void runGuarded(async () => {
    ...
    if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
      throw new Error('Loaded collision decisions file must contain a JSON object');
    }
    ...
    return { loaded: true, path: selected, keys: Object.keys(parsed as Record<string, unknown>).length };
  });
});

document.querySelector<HTMLButtonElement>('#validate-btn')?.addEventListener('click', () => {
  void runGuarded(async () => {
    if (!sourcePath) {
...
document.querySelector<HTMLButtonElement>('#normalize-preview-btn')?.addEventListener('click', () => {
  void runGuarded(async () => {
    if (!sourcePath) {
      throw new Error('Select source folder first');
    }
...
document.querySelector<HTMLButtonElement>('#install-dry-run')?.addEventListener('click', () => {
  void runGuarded(async () => {
...
document.querySelector<HTMLButtonElement>('#install-plan')?.addEventListener('click', () => {
  void runGuarded(async () => {
    if (!sourcePath || !targetPath) {
      throw new Error('Select source and target folders first');
    }
...
document.querySelector<HTMLButtonElement>('#install-apply')?.addEventListener('click', () => {
  void runGuarded(async () => {
    if (!sourcePath || !targetPath) {
      throw new Error('Select source and target folders first');
    }
...
document.querySelector<HTMLButtonElement>('#doctor-btn')?.addEventListener('click', () => {
  void runGuarded(async () => {
    if (!targetPath) {
...
document.querySelector<HTMLButtonElement>('#readiness-btn')?.addEventListener('click', () => {
  void runGuarded(async () => {
    if (!targetPath) {
      throw new Error('Select target folder first');
    }
...
document.querySelector<HTMLButtonElement>('#registry-btn')?.addEventListener('click', () => {
  void runGuarded(async () => {
    if (!targetPath) {
      throw new Error('Select target folder first');
    }
